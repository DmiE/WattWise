...
- [ ] 4.1 `npm test` passes
- [ ] 4.2 Relaxing any single bound in `onboarding-schema.ts` by one step turns exactly that row red
- [ ] 4.3 Deleting the uniqueness `.refine` turns the duplicate-days test red
- [ ] 4.4 Deleting `.min(1)` on `available_days` turns the empty-array test red
- [ ] 4.5 Deleting `.int()` from `age` turns the fractional-age test red
- [ ] 4.6 `npm run lint` and `npm run build` pass

#### Manual

- [ ] 4.7 Every bound is verified against `init_mvp_schema.sql:50-59` and cites its line
- [ ] 4.8 No value is imported, computed, or derived from `onboarding-schema.ts`
- [ ] 4.9 The `weight_kg` probes are `200.004` / `29.996` / `70.123456`
- [ ] 4.10 The `available_days` cases state this is sole-enforcement, not parity, and name B1/B2
