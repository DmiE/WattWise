...
|---|---|---|---|---|---|---|
| 1 | Runner bootstrap + trust-boundary units | Stand up a test runner on the workerd-targeted stack and prove the AI trust boundary and equipment mapping reject what they must | #1, #3, #6 | unit | change opened | `context/changes/testing-runner-bootstrap/` |
...
|---|---|---|---|
| unit + integration | none yet — see §3 Phase 1 | — | Runner choice must be verified against Astro 6 + Vite 7 + the workerd runtime target before adoption; do not assume a config from another Astro project transfers |
...
- TBD — see §3 Phase 1. Will carry the pattern for asserting that an
  invalid generated plan is rejected rather than persisted, sourcing
  expected values from PRD business rules rather than from the validator.
